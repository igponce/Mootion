...
foreach ($negatives as $negative) {
	...
	$positive_count = $db->get_var("select count(*) from votes where vote_type='links' and vote_link_id = $linkid and vote_value > 0");	
	$positive_users_count = $db->get_var("select count(*) from votes where vote_type='links' and vote_link_id = $linkid and vote_user_id > 0 and vote_value > 0");	
	$positive = $db->get_var("select sum(vote_value) from votes where vote_type='links' and vote_link_id = $linkid and vote_value > 0");	
	$positive_users = $db->get_var("select sum(vote_value) from votes where vote_type='links' and vote_link_id = $linkid and vote_user_id > 0 and vote_value > 0");	
	...
	$negative_users_count = $db->get_var("select count(*) from votes, users where vote_type='links' and vote_link_id = $linkid and vote_user_id > 0 and vote_value < 0 and user_id = vote_user_id");	
	$negative_users = $db->get_var("select sum(vote_value-user_karma) from votes, users where vote_type='links' and vote_link_id = $linkid and vote_user_id > 0 and vote_value < 0 and user_id = vote_user_id");	
