...
sub crossed_votes {
	...
	#	print "$userid, $user_votes{$userid}: ";
		...
		while( ($linkid, $value) = $sth->fetchrow_array ) {
			...
			$sql = "select distinct vote_user_id, vote_value from votes where vote_link_id = $linkid and vote_user_id > 0 and vote_user_id != $userid and vote_date > FROM_UNIXTIME($from)";
...
sub link_votes {
	...
	# Each voted link
	$sql = "select vote_user_id, link_author, vote_value from votes, links where vote_date > FROM_UNIXTIME($from) and vote_user_id > 0 and link_id = vote_link_id";
	$sth = $dbh->prepare($sql);
	$sth->execute();
