...
if(!empty($_REQUEST['time'])) {
	...
	$last_modified = time();
	$title = 'mOOtion.com most voted in ' . ' ' . txt_time_diff($from);
	//$link_date = "modified";
	$link_date = "";
} else {
	// All the others
	...
	if($search) {
		$from_where .= $search;
		$title =  "mOOtion.com: " . htmlspecialchars($_REQUEST['search']);
	}
...
if ($links) {
	foreach($links as $link_id) {
		...
		echo "		<comments>". $link->get_permalink() ."#comments</comments>\n";
		if (!empty($link_date))
		...
		// ------------------------------
		// Incrustar contenido completo
		// ------------------------------
		
		echo '		<description><![CDATA[';
		// Si link->embedhtml!=null... *INCRUSTAMOS* el video.

		$linktarget = '';
		$url = htmlspecialchars($link->url);
		$url_short = htmlspecialchars(txt_shorter($link->url));
		$title_short = htmlspecialchars(wordwrap($link->title, 36, " ", 1));
		
		if ( $link->embedhtml ) {
			echo '<div class="news-summary">';
			echo '<h3 id="title'.$link->id.'">';
			echo '<a href="'. $link->get_permalink() .'">'. $title_short. '</a></h3>' ."\n";
			echo html_entity_decode ($link->embedhtml, ENT_COMPAT);
			echo $content;
			echo '<br/>sent by <a href="user.php?login='.$link->username.'&amp;view=history"'.$linktarget.'><strong>'.$link->username.'</strong></a> ';
		} else {
	echo '<div class="news-summary">';
			echo '<h3 id="title'.$link->id.'">';
			echo '<a href="'. $link->get_permalink() .'">'. $title_short. '</a></h3>' ."\n";
...
function do_header($title) {
	...
	global $rss_server_name, $rss_languaje, $msg_rssdescription;
	...
	echo"	<image><title>mOOtion</title><link>http://mootion.com</link><url>http://mootion.com/img/es/logo01.png</url></image>\n";
	echo'	<description>'.$msg_rssdescription.'</description>'."\n";
	echo'	<pubDate>'.date("r", $last_modified).'</pubDate>'."\n";
	echo'	<generator>mOOtion RSS2 writer</generator>'."\n";
...
function check_redirect_to_feedburner($status) {
	...
	switch ($status) {
		...
		case 'queued':
			header("Location: http://feeds.feedburner.com/mootion/queued");
			exit();
			break;
		****/
		case 'all':
