...
if(is_numeric($_REQUEST['id'])) {
	...
	// videofarm-specific code goes here
	...
	if($link->date < time()-345600) { // older than 4 days
		...
		echo '<div class="air-with-footer">'."\n";
		echo '<div class="commentform" align="center" >'."\n";
		echo '<a href="/login.php?return='.$_SERVER['REQUEST_URI'].'">'. 'Autentifícate si deseas escribir ' .'</a> '.'comentarios'.'O regístrate' . ' <a href="/register.php">aquí</a>.'."\n";
		echo '</div>'."\n";
		echo '</div>'."\n";
	}
...
function print_comment_form() {
	...
	echo '<fieldset><legend><span class="sign">'. 'env&iacute;a un comentario' .'</span></legend>'."\n";
	//echo '<p>'."\n";
	echo 'Sé respetuoso, no insultes, exprésate con el voto. Si la noticia o un comentario te parece fuera de lugar, explícalo de forma breve y objetiva. Se agradece el buen humor.';
	echo '<label for="comment" accesskey="2" style="float:left">'. 'texto del comentario / no se admiten etiquetas HTML' .'</label>'."\n";
	//echo '</p>';
	echo '<p class="l-top-s"><br/>'."\n";
	echo $foo;
	echo '<textarea name="comment_content" id="comment" rows="3" cols="76"/></textarea><br/>'."\n";
	echo '<input class="submitcomment" type="submit" name="submit" value="'. 'enviar el comentario' .'" />'."\n";
