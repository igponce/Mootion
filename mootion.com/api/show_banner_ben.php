...
if ($res) {
	...
	$title['most'] = cut($res->link_title) . " [".$votes_hour." ". 'votos/hora' ."]";
...
if ($res) {
	$title['published'] = cut($res->link_title) . " [".$res->link_votes." ". 'votos' ."]";
...
if ($res) {
	$title['sent'] = cut($res->link_title) . " [".$res->link_votes." ". 'votos' ."]";
...
<?echo $div3;?>
<a href="<? echo $url['most']?>" style="color: #<? echo $color_link;?>" target="_parent"><? echo 'Most recently mOOved'; ?></a><br />
<? echo $title['most'] ?>
</div>
