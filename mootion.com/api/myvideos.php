...
// Portions Copyright 2006 - Inigo Gonzalez <igponce at corp dot mootion dot com>
...
if ( ! is_numeric ($width) ) {
    $width = 680; // defaults to horizontal style
}

if ( ! is_numeric ($height) ) {
    $height = 80; // defaults to horizontal style
}
...
if ($res) {
	$title['voted'] = cut($res->link_title) . ' <span style="font-size: 90%;">['.$res->link_votes."&nbsp;". 'votes' ."]</span>";
...
switch ($format) {
	case 'vertical':
		...
		$signature = '<img src="http://'.get_server_name().'/img/common/mootion-10px.png" border=0>';
		break;
	case 'horizontal':
	default:
		...
		$signature = 'latest videos from <img src="http://'.get_server_name().'/img/common/mootion-10px.png" border=0>';
}
